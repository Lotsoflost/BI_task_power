/*===============================================================================
URL..................: adventure_works_test/views/mv_sales_datamart_all_joint
Activity.............: BI - Materialized view for Sales Datamart (joined star)
Description..........:
    Pre-joined snapshot of fact and dimensions for fast analytical reads.
    Contains only current valid SCD slices per order date.

Owner................: Olga Pankova
Refresh strategy.....: Manual or scheduled; supports CONCURRENTLY
===============================================================================*/

-- 1) Create materialized view (empty for fast DDL, then refresh)
DROP MATERIALIZED VIEW IF EXISTS adventure_works_test.mv_sales_datamart_all_joint CASCADE;

CREATE MATERIALIZED VIEW adventure_works_test.mv_sales_datamart
AS
...
ALTER MATERIALIZED VIEW adventure_works_test.mv_sales_datamart OWNER TO postgres;

-- 2) Indexes (unique for CONCURRENT refresh + a few helpful filters)
CREATE UNIQUE INDEX IF NOT EXISTS ux_mv_salesorderline
    ON adventure_works_test.mv_sales_datamart (salesorderlinekey);

CREATE INDEX IF NOT EXISTS ix_mv_date
    ON adventure_works_test.mv_sales_datamart (orderdatekey);


CREATE INDEX IF NOT EXISTS ix_mv_region
    ON adventure_works_test.mv_sales_datamart (region_territory);


-- (Optional) Comments
COMMENT ON MATERIALIZED VIEW adventure_works_test.mv_sales_datamart
IS 'Pre-joined sales datamart snapshot for BI queries (supports CONCURRENT refresh).';
